...
Usage:
    python 01_run_simulation_replicates.py --outdir out --n_samples 50 --generations 200
...
class PfSimParams:
    ...
    #rec_rate: float = 6.67e-7       # per bp per generation      7.4e-8
...
class PfSimulation:
    ...
    def write_slim_script(self, chr_idx: int, rec_rate: float, output_file: str): # , tracking_output: str
        """
...
{{
    ...
    defineConstant("L", {L});                                                     // chromosome length in bp  
  	...
  	defineConstant("outid", 1);                                                   // output ID for saving state
...
"""
    ...
    def run_slim_simulation(self, chr_idx: int, rec_rate: float, prefix: str, output_dir: str = "pf_simulation_output") -> str:
        ...
        with open(script_file, 'w') as f:
            f.write(self.write_slim_script(chr_idx, rec_rate, output_file)) # , tracking_output
        ...
        with open(out_file, "w") as out, open(err_file, "w") as err:
            # with shell=True:
            result = subprocess.run(f"slim.exe {script_file}", shell=True, stdout=out, stderr=err, text=True)
    ...
    def simplify_to_samples(self, ts: tskit.TreeSequence) -> tskit.TreeSequence:
        ...
        n_sample = self.params.n_samples // 2                             # Haploid samples, so half the individuals
        # n_sample = min(self.params.n_samples, alive_idx)                # In case alive individuals are less than requested samples
        print(f" Alive individuals: {alive_idx}, Sampling {n_sample} individuals")
